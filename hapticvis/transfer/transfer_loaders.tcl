...
namespace eval hapticvis::transfer {
	...
	$s add_method add_subject_blocks { subject_id block_id \
					       dbfile shapedb } \
	    {
	...
	$s add_method setup_visual_cued { subject_id subject_set n_per_set \
		...
		set lr_ecc 6.0
		set lr_scale 1.75
		dl_set stimdg:lr_choice_centers \
		    [dl_replicate \
			 [dl_llist [dl_llist \
					[dl_flist -$lr_ecc 0] \
					[dl_flist $lr_ecc 0]]] $n]
	...
	$s add_method setup_visual_transfer \
	...
	$s add_method setup_haptic_transfer \
	    { subject_id subject_set n_per_set \
