...
proc do_world_jitter { source dest } {
...
proc jitter_world { dg id n } {
	...
	box2d::destroy all
	set jittered_world [do_world_jitter $w $jittered_world]
...
proc do_ball_jitter { source dest } {
    set translate_scale 1.0
    set rotate_scale [expr {$::pi/180.*10}]
    dl_local is_ball [dl_regmatch $source:name ball*]
    set n [dl_length $is_ball]
    dl_local tx [dl_add $source:tx [dl_mult [dl_sub [dl_urand $n] 0.5] \
					$translate_scale $is_ball]]
    dl_local ty [dl_add $source:ty [dl_mult [dl_sub [dl_urand $n] 0.5] \
					$translate_scale $is_ball]]
    dl_local a [dl_add $source:angle [dl_mult [dl_sub [dl_urand $n] 0.5] \
					  $rotate_scale $is_ball]]
    dl_set $dest:tx $tx
    dl_set $dest:ty $ty
    dl_set $dest:angle $a
    return $dest
...
proc jitter_ball { dg id n } {
    set w [get_world $dg $id]
    set jittered_world [dg_copy $w]

    for { set i 0 } { $i < $n } { incr i } {
	box2d::destroy all
	set jittered_world [do_ball_jitter $w $jittered_world]
	lassign [planko::build_world $jittered_world] world ball
	set s [run_simulation $world $ball]
...
proc ball_jitter_dtw { dg njitter from to { skip 1 } } {
    set out [dg_create]
	...
	set j [jitter_ball $dg $i $njitter]
...
proc world_jitter_dtw { dg njitter from to { skip 1 } } {
    set out [dg_create]
    foreach c "t x y jx jy angle distance distance_matrix" {
	dl_set $out:$c [dl_llist]
    }
...
proc ball_jitter { dg njitter from to } {
    set out [dg_create]
    foreach c "t x y jx jy angle distance distance_matrix" {
	dl_set $out:$c [dl_llist]
    }
...
proc world_jitter { dg njitter from to } {
    set out [dg_create]
    foreach c "t x y jx jy angle distance distance_matrix" {
	dl_set $out:$c [dl_llist]
    }
...
proc trial_world_jitter { dg njitter trial } {
    set out [dg_create]
    foreach c "t x y jx jy angle distance distance_matrix" {
	dl_set $out:$c [dl_llist]
    }
    ...
    set j [jitter_world $dg $trial $njitter]
    foreach jc "t x y jx jy angle" {
	dl_append $out:$jc $j:$jc
    }
    dl_append $out:nhit [dl_get $dg:nhit $i]
...
proc trial_ball_jitter { dg njitter trial } {
    set out [dg_create]
    foreach c "t x y jx jy angle distance distance_matrix" {
	dl_set $out:$c [dl_llist]
    }
    ...
    set j [jitter_ball $dg $trial $njitter]
    foreach jc "t x y jx jy angle" {
	dl_append $out:$jc $j:$jc
    }
    dl_append $out:nhit [dl_get $dg:nhit $i]
