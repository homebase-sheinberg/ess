...
namespace eval match_to_sample::phd {
    ...
    proc loaders_init { s } {
        ...
        $s add_method get_shape_qrs { db shapeID filled { displayID 51 } } {
            ...
            if { $filled } {
                set sqlcmd {SELECT hex(shapeFilled) FROM shapeTable  WHERE shapeID=$shapeID and DisplayID=$displayID}
            } else {
                set sqlcmd {SELECT hex(shapeOutline) FROM shapeTable  WHERE shapeID=$shapeID and DisplayID=$displayID}
            }
            ...
            set bits [format %032b%032b%032b%032b  0x${word0} 0x${word1} 0x${word2} 0x${word3}]
        ...
        $s add_method setup_trials { dbfile trial_type filled limit } {
            ...
            dl_local distance [dl_choose [dl_replicate [dl_shuffle "2 4 8 16"] $nrep]  [dl_fromto 0 $nfamilies]]
            ...
            dl_set stimdg:sample_slot [dl_unpack  [dl_indices [dl_eq stimdg:choice_ids stimdg:sample_id]]]
            grasp_db close
